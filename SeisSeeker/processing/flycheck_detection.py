...
def _fast_freq_domain_array_proc(
    data, max_sl, fs, target_freqs, xx, yy, n_stations, n_t_samp, remove_autocorr
):
    ...
    # Create data stores:
    Pfreq_all = np.zeros(
        (data.shape[0], len(target_freqs), n_ur, n_utheta), dtype=np.complex128
...
def _phase_associator(
    ...
    for event_idx in range(len(Z_hor_phase_pair_idxs)):
        ...
        curr_event_df = pd.DataFrame(
            {
                "t1": [t_series_df_Z["t"][curr_peak_Z_idx]],
                "t2": [t_series_df_hor["t"][curr_peak_hor_idx]],
                "pow1": [t_series_df_Z["power"][curr_peak_Z_idx]],
                "pow2": [t_series_df_hor["power"][curr_peak_hor_idx]],
                "slow1": [t_series_df_Z["slowness"][curr_peak_Z_idx]],
                "slow2": [t_series_df_hor["slowness"][curr_peak_hor_idx]],
                "bazi1": [t_series_df_Z["back_azi"][curr_peak_Z_idx]],
                "bazi2": [t_series_df_hor["back_azi"][curr_peak_hor_idx]],
            }
        )
        events_df = events_df._append(curr_event_df)
    # And tidy:
        ...
        if len(events_df) > 0:
            ...
            for index, row in events_df.iterrows():
                ...
                if tmp_count == 1:
                    ...
                    if (
                        obspy.UTCDateTime(row["t1"])
                        - obspy.UTCDateTime(tmp_df["t1"].iloc[0])
                        < min_event_sep_s
                    ):
                        ...
                        combined_pows_tmp = (
                            tmp_df["pow1"].values + tmp_df["pow2"].values
                        )
                        max_power_idx = np.argmax(combined_pows_tmp)
                        filt_events_df = filt_events_df._append(
                            tmp_df.iloc[max_power_idx]
                        )
            ...
            max_power_idx = np.argmax(combined_pows_tmp)
            filt_events_df = filt_events_df._append(tmp_df.iloc[max_power_idx])
            ...
            filt_events_df = filt_events_df.sort_values("sum_pows").drop_duplicates(
                subset="t2", keep="last"
            )
...
class setup_detection:
            ...
            for j in range(self.n_stations):
                ...
                try:
                    if win_end_idx < len(
                        ...
                        data[i, j, :] = 0.0
                        print(
                            "Warning: Zero-padding as not enough data to fill window overlap ( for win_len_s =",
                            self.win_len_s,
                            "and win_step_inc_s =",
                            self.win_step_inc_s,
                            ")",
            ...
            for julday in range(1, 367):
                ...
                for self.channel_curr in self.channels_to_use:
                    ...
                    for hour in range(24):
                        ...
                        if (
                            self.starttime
                            > obspy.UTCDateTime(year=year, julday=julday, hour=hour)
                            + 3600
                        ):
                            continue
                        if self.endtime < obspy.UTCDateTime(
                            year=year, julday=julday, hour=hour
                        ):
                            continue

                        # Create datastore:
                        out_df = pd.DataFrame(
                            {"t": [], "power": [], "slowness": [], "back_azi": []}
                        )
                        ...
                        for minute in range(60):
                            ...
                            if self.endtime - self.starttime > 60:
                                for t_serie in t_series:
                                    t_series_out.append(
                                        str(starttime_this_st + (minute * 60) + t_serie)
                                    )
                            else:
                                for t_serie in t_series:
                                    t_series_out.append(
                                        str(starttime_this_st + t_serie)
                                    )
                            tmp_df = pd.DataFrame(
                                {
                                    "t": t_series_out,
                            ...
                            out_fname = os.path.join(
                                ...
                                "".join(
                                    (
                                        ...
                                        str(julday).zfill(3),
                                        "_",
                                        str(starttime_this_st.hour).zfill(2),
                                        "00",
                                        "_ch",
                                        self.channel_curr[-1],
                                        ".csv",
    ...
    def _calc_uncertainties(
        ...
        for index, row in events_df.iterrows():
            ...
            if count == 0:
                st = self._load_day_of_data(
                    event_phase_arr_time.year,
                    event_phase_arr_time.julday,
    ...
    def get_composite_array_st_from_bazi_slowness(
        ...
        st = self._load_day_of_data(
            arrival_time.year, arrival_time.julday, hour=arrival_time.hour
        )
        # And trim data:
